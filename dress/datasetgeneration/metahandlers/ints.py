...
from dress.datasetgeneration.grammars.utils import random_seq
...
class IntListExcludingSomeValues(MetaHandlerGenerator):
    ...
    def __init__(
        ...
        exclude: Union[int, Iterable[int], Iterable[range]],
    ):
        ...
        if isinstance(exclude, int):
            ...
            self.exclude = set(
                chain(*[x if isinstance(x, Iterable) else [x] for x in exclude])
            )
    ...
    def generate(
        ...
        assert (
            possible_values
        ), f"no possible values left after excluding {self.exclude}"
...
class IntRangeExcludingSomeValues(MetaHandlerGenerator):
    ...
    def __init__(
        self, min: int, max: int, exclude: Union[int, Iterable[int], Iterable[range]]
    ):
...
class CustomIntListDeletions(MetaHandlerGenerator):
    ...
    def generate(
        ...
        # Return a random sequence of the same size of deletion
        if self.is_ablation:
            rec((_selected[0], _selected[1], i, random_seq(_selected[5][1], rs=r)))
        else:
